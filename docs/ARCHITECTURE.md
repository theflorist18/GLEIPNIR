...
│   ├── networks/                     # coc-main.yaml, connection-org1.yaml, rest-gateway.yaml (parallel-c{C} rendered per run into the run dir by rounds.py)
...
│   └── README.md
├── frontend/                         # Evidence-library SPA (M14): auth/, components/, pages/{admin,investigator,lead}
│   └── README.md
...
**Caliper attachment.** Caliper connects as a gateway client from Org1 using the User1 identity; its network config references the peer's TLS CA cert, the client signed cert + key, and the target channel(s). For multi-channel (Parallel) runs, one network config listing all C case channels (`parallel-c{C}.yaml`) is rendered per run into the run dir by `orchestration/rounds.py`.
...
**Responsibility:** automate per-case channel provisioning (Caliper configs are rendered per run by `rounds.py` / `experiment.py`).

Interface (shell/Python):
```
provision-channel.sh <caseId>   # configtxgen genesis → osnadmin channel join (expect HTTP 201)
                                 # → peer join → approve/commit chaincode (install is org-scoped, done once by up.sh)
```
...
POST/PATCH     /evidence-index[/:evidenceId]         # register row / PATCH /:evidenceId; GET /evidence-index?… = search with visibility scoping (no single-row GET)
...
GET    /blobs/:evidenceId          # attachment stream
DELETE /blobs/:evidenceId          # ingest-failure rollback ONLY (x-gleipnir-rollback-token from the PUT)
```
...
| investigator | `EvidenceDetailPage` | `EvidenceCard` + `AuditTrail` + `MerkleBadge` + download/export + transfer/access forms; opening it demonstrates the server-side auto-`AccessLog(view)`; `MerkleBadge`/Verify active once a session write returns `batched: true` (N/A on the direct variants) |
| investigator | `SearchPage` | evidence-index + case search, participant-scoped server-side |
| admin | `UsersPage`, `CasesAdminPage` | user management (deactivate-not-delete); all-cases list, case status change, categorize/uncategorize (admins create cases from `MyCasesPage`'s New-case modal and grant/manage rosters in `CaseDetailPage`'s Team section) |
...
sequenceDiagram
  ...
  participant G as API Gateway
  participant P as Peers on Hyperledger Fabric 2.5 LTS
  O->>P: osnadmin channel join (HTTP 201) + approve/commit  [per case; install once per org]
...
- **Standard / Parallel** (`mode: fabric`): workloads call `sendRequests({contractId:'evidence', contractFunction, contractArguments, invokerIdentity:'User1', readOnly, channel?})`; for Parallel, the per-channel network config `parallel-c{C}.yaml` is rendered by `orchestration/rounds.py` (`render_parallel_network`) into each run dir and passed to Caliper as an absolute path (nothing generated is committed), load is spread round-robin across `case-001..case-00C` in ONE round, and results are reported aggregate + derived per-channel.
...
| 7 | Provisioning automation (Parallel) | `provision-channel.sh` stands up channels for sweep {1,2,5}; per-channel Caliper configs generated (since 2026-09-26 rendered per run by `rounds.py`; `provision-channel.sh` no longer emits them) |
...
| 17 | User `name` rename (library) | `displayName` → `name` across store/API/SPA (CONTRACTS §12-8); gateway tests + `smoke-library.sh` pass (the boot-time pre-M17 `users.json` upgrade was removed 2026-09-26 as a spent one-time migration, CONTRACTS §12-20) |
| 18 | 3-tier RBAC (library) | `lead` role lands (user + case level, CONTRACTS §12-8): a lead creates a case and manages its roster; an investigator cannot create; admin `GET .../download` off-case → 403 while view/audit/export stay 200; `smoke-library.sh` steps 12–14 pass (the one-time in-place CHECK rebuild for a pre-M18 `case-registry.db` was removed 2026-09-26, CONTRACTS §12-20) |
